...
(function( $ ){
	...
	finished : function (callback) {
		...
		var el 			 	= $(this),
			...
			if(eventData.end.speed.y > 0.85 && eventData.delta.dir.y == -1){
				
				var offsetSegs = eventData.end.speed.y < 1.2 ? 25 : 35;
				
				el.flickable('segment',segment+offsetSegs<segments?segment+offsetSegs:segments);
				return;
			}else if(eventData.end.speed.y > 0.85 && eventData.delta.dir.y == 1) {
				
				var offsetSegs = eventData.end.speed.y < 1.2 ? 25 : 35;
				el.flickable('segment',segment-offsetSegs>=0?segment-offsetSegs:0);
				return;
			}
